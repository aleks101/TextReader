...
Text::Text(Shader* shader) : m_shader(shader){

}
Text::~Text(){

}
void Text::LoadFont(const char* fontPath, int size){
    // FreeType
    // --------
    FT_Library ft;
...
void Text::Render(std::string text, Vec2 pos, float scale, ColorRGB color){
    // activate corresponding render state	
    m_shader->Use();
    glUniform3f(glGetUniformLocation(m_shader->GetID(), "textColor"), color.R, color.G, color.B);
    glm::mat4 m_translation = glm::mat4(1.0f);
    m_translation = glm::translate(m_translation, glm::vec3((float)pos.x, (float)pos.y, 0.0f));
